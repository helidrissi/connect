 ...
 *
 */
public class ConvertCbl2RecordEditorXml03 {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	LayoutDetail inLayout = extlayoutCBL.asLayoutDetail();

    	for (int i = 0; i < extlayoutCBL.getNumberOfRecordFields(); i++) {
    		ExternalField recordField = extlayoutCBL.getRecordField(i);
			if ("KEYCODE-NO".equalsIgnoreCase(recordField.getName())) {
    			System.out.println("Found: " + i);
    			recordField.setType(Type.ftNumZeroPadded);
    		}
    	}
       	LayoutDetail outLayout = extlayoutCBL.asLayoutDetail();
   	
    	CopybookWriterManager.getInstance()
    			.get(CopybookWriterManager.RECORD_EDITOR_XML_WRITER)
    		    .writeCopyBook(
    		    		new FileOutputStream("H:\\Temp\\XML_DTAR020c.Xml"), 
    		    		extlayoutCBL, 
    		    		null);
    }
}
