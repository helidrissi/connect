	...
	<fo:page-sequence master-reference="master-page">
		...
		<xsl:apply-templates select="enteteliste"/>		<!--Sélectionne et affiche l'entête de la page-->
			...
			<fo:flow flow-name="xsl-region-body" font-size="9pt">		<!--Taille de la police pour l'espace de travail-->
				<fo:table table-layout="fixed" border-style="solid 0.3mm" border-width="0.0mm" width="157mm">		<!--Taille du tableau-->
				  <fo:table-column background-color="white"  color="white" column-width="22mm" />
					...
					<fo:table-header>
						<fo:table-row line-height="20pt" background-color="{$colorbleu}" color="black" font-weight="bold">
						  <fo:table-cell color="white" background-color="white"> <fo:block> . </fo:block> </fo:table-cell>
							...
							<fo:table-cell margin-left="0.5mm" margin-right="0.5mm" border="solid 0.3mm" text-align="center"> <fo:block> Libellé Langue </fo:block> </fo:table-cell>
						</fo:table-row>
					</fo:table-header>
					<fo:table-footer>
					</fo:table-footer>
					<fo:table-body>
						<xsl:apply-templates select="lan"/>
					</fo:table-body>
				</fo:table>
				<fo:block id="Fin"/>
			</fo:flow>
